export class VanityKeyGenerator {
  ...
  /** Only the first 4 bytes of the key are compared, so 8 hex chars is the cap. */
  static get maxPrefixLength() {
    return 8;
  }
   ...
   * @param {string} prefix - hex prefix to match (1-8 chars)
  ...
  generate(prefix) {
    ...
    if (prefix.length === 0 || prefix.length > VanityKeyGenerator.maxPrefixLength) {
      throw new Error(`Prefix must be 1-${VanityKeyGenerator.maxPrefixLength} hex characters`);
    }
    // MeshCore's validatePrivateKey() rejects public keys starting with 00 or FF
    const firstByte = prefix.slice(0, 2).toLowerCase();
    if (firstByte === '00' || firstByte === 'ff') {
      throw new Error('Public keys starting with 00 or ff are rejected by MeshCore');
    }
